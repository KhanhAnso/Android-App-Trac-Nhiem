...
import com.example.apptracnghiem.Table;
...
public class Database extends SQLiteOpenHelper {
    ...
    //đổi version cập nhật lại lần thứ 2 , tất cả dữ liệu cũ được giữ lại , chỉ việc tăng version nếu muốn cập nhật không bị mất dữ liệu
    ...
    private void insertCategory(Category category) {
        ...
        cv.put(Table.CategoriesTable.COLUMN_NAME, category.getName());
        db.insert(Table.CategoriesTable.TABLE_NAME, null, cv);
    }
    //Cac gia tri insert
    ...
    private void insertQuestion(Question question) {
        ContentValues cv = new ContentValues();
        cv.put(Table.QuestionsTable.COLUMN_QUESTION, question.getQuestion());
        cv.put(Table.QuestionsTable.COLUMN_OPTION1, question.getOption1());
        cv.put(Table.QuestionsTable.COLUMN_OPTION2, question.getOption2());
        cv.put(Table.QuestionsTable.COLUMN_OPTION3, question.getOption3());
        cv.put(Table.QuestionsTable.COLUMN_OPTION4, question.getOption4());
        cv.put(Table.QuestionsTable.COLUMN_ANSWER, question.getAnswer());
        cv.put(Table.QuestionsTable.COLUMN_CATEGORY_ID, question.getCategoryID());
        db.insert(Table.QuestionsTable.TABLE_NAME, null, cv);
    }
    //các tạo các bản ghi câu hỏi
    ...
    //Lấy dữ liệu câu hỏi và đáp án thuộc chủ đề có id = categoryID trong SQLite gán vào mảng questionList
